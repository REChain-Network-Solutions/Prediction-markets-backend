    ...
    async init() {
        ...
        if (this.balance <= 1e6) {
            throw clc.red.bold(`
                ...
                ##################################################################################
                ${address} balance: ${+Number(this.balance / 1e9).toFixed(9)} GREChain. ${balance?.base?.pending ? `(Pending: ${+Number((balance?.base?.pending || 0) / 1e9).toFixed(9)} GREChain)` : ''}
                Your wallet contains less than 0.01 GREChain.
            ...
            console.error(clc.green.bold(`
                ################################################################
                ${address} balance: ${+Number(this.balance / 1e9).toFixed(9)} GREChain.
                ################################################################
                `))
        }
